-- ============================================================
-- Capacitaciones: promedio real por capacitación + máximo entre ellas
-- ============================================================
-- Edward pidió una regla general nueva para "Capacitaciones y formación
-- continua" (5%): si un profesor participa de varias capacitaciones en
-- el mismo período, el resultado final es el MEJOR de todas, no el
-- promedio de todas (como hacía migracion_eventos_criterios.sql).
--
-- Además, una capacitación con varios encuentros reales (temas
-- distintos, no la misma reunión repetida por sede) debe puntuar como
-- el PROMEDIO de asistencia a esos encuentros, no como "asistió a
-- cualquiera de ellos = 100%" (eso queda reservado para reuniones que
-- se repiten idénticas por sede, que sigue siendo un caso aparte:
-- ver OBJETIVO_REUNIONES).
--
-- Cambios respecto a migracion_eventos_criterios.sql:
--   - Dentro de cada grupo de capacitación no evaluada: en vez de
--     bool_or (asistió a alguna = 100%), ahora es
--     count(asistidas) / count(encuentros del grupo) * 100.
--   - Entre las distintas capacitaciones del período: en vez de
--     avg(puntaje), ahora es max(puntaje).
--
...
select
  ...
  case cr.origen
    ...
    when 'OBJETIVO_CAPACITACIONES' then
      round(coalesce(cap.mejor_pct, 0) * cr.peso_porcentaje / 100, 2)
    else
...
left join lateral (
  select max(x.pct) as mejor_pct
  from (
    select
      case
        when bool_and(e.tipo = 'CAPACITACION_NO_EVALUADA')
          then count(ear.id)::numeric / count(*) * 100
...
select registrar_migracion('migracion_capacitaciones_maximo.sql');
